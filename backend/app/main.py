...
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, status, Response, BackgroundTasks
...
def check_critical(h_input: str, user_collection):
    keywords = ["allergy", "allergic", "allergen", "allergens", "chronic", "diabetes", "cancer", "cancers", "heart", "asthma",
                "disease", "addict", "addiction", "addicted", "condition", "blood pressure", "bp", "deficiency", "blood cells", "cell",
                "cells"]
...
def get_language_name(code: str) -> str:
    lang_map = {
        ...
        "pa": "Punjabi", "or": "Odia", "ur": "Urdu", "en-IN": "English", "en": "English"
    }
    if not code:
        return "English"
...
def speech_to_text(audio_data: bytes) -> tuple:
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Sarvam STT failed with status {response.status_code}: {response.text}")
...
async def process_emergency(
    ...
    if client and user_client:
        ...
        try:
            user_collection = user_client.get_or_create_collection(name="userdata", metadata={"hnsw:space": "cosine"})
        except Exception as e:
    ...
    if context_collection and detected_text:
        try:
            ...
            if results and results.get('distances') and results['distances'][0][0] < RAG_RELEVANCE_THRESHOLD:
    ...
    if user_collection and detected_text and keyword:
        try:
            user_results = user_collection.query(query_texts=[detected_text], n_results=1)
    ...
    try:
        ...
        output = chain.invoke({
            ...
            "question": human_content if image else f"Current question: {detected_text}",
            "language": language_name,
            "history": current_history
        })
        
        text_response = output.content[0]['text'] if isinstance(output.content, list) else str(output.content)
        ...
        def dispatch_whatsapp():
            try:
                summary_prompt = f"Summarize the following emergency situation and response in 2 concise sentences for an emergency contact. VERY IMPORTANT: Refer to the victim as '{username}' instead of 'I' or 'the victim'.\nSituation: {detected_text}"
                whatsapp_summary_res = g_model.invoke([HumanMessage(content=summary_prompt)])
                summary_text = whatsapp_summary_res.content
                if isinstance(summary_text, list):
                    summary_text = " ".join([part.get("text", "") for part in summary_text if isinstance(part, dict) and "text" in part])
...
async def call_ambulance_endpoint(background_tasks: BackgroundTasks, username: str = "User"):
    ...
    def dispatch_ambulance_whatsapp():
        try:
            summary_prompt = f"Generate a short, urgent 2-sentence WhatsApp alert stating that an ambulance has been called and dispatched to {username}'s location. Refer to them as {username}."
            whatsapp_summary_res = g_model.invoke([HumanMessage(content=summary_prompt)])
            summary_text = whatsapp_summary_res.content
            if isinstance(summary_text, list):
                summary_text = " ".join([part.get("text", "") for part in summary_text if isinstance(part, dict) and "text" in part])
...
async def text_to_speech_endpoint(req: TTSRequest):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Sarvam TTS failed with status {response.status_code}: {response.text}")
...
async def tts_stream_endpoint(text: str, background_tasks: BackgroundTasks, language_code: str = "en-IN"):
    if not SARVAM_API_KEY:
        raise HTTPException(status_code=500, detail="Sarvam API Key not configured")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            temp_file_path = os.path.join(tempfile.gettempdir(), f"resq_tts_{uuid.uuid4()}.wav")
